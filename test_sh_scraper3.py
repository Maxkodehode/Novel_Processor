...
log_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test_output2.log")
...
log("=== ScribbleHub Scraper Test (Round 2) ===")
log(f"PID: {os.getpid()}")
...
log("JSON saved to output2.json")
...
if len(orders) != len(set(orders)):
    ...
    dupes = {k: v for k, v in Counter(orders).items() if v > 1}
    for order, count in sorted(dupes.items())[:10]:
        log(f"  Order {order}: {count} chapters")
else:
    log("No duplicate orders.")
